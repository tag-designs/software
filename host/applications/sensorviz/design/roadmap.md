...
- `sqlite_loader.*`: read-only SQLite adapter. It consumes tagcore's mandatory
  ...
  metadata, and records collection-start metadata for elapsed-time IMUTag logs.
...
- `stream_actions.cpp`: visible-stream dialog, axis-side dialog, color dialog,
  range dialog, derived-stream ordering, and range coupling between related
  streams.
...
- `interaction.cpp`: context menu, print preview, cursor interaction, draggable
  metadata box behavior, UTC offset, and mouse readout.
...
- Activity and activity-filter ranges are coupled unless one has an explicit
  custom range.
- IMUTag raw component streams default hidden on the right axis; derived
  magnitudes and altitude default visible on the left.
- The plot metadata box is contextual and draggable. Its position is session
  state; no preference file key currently preserves it.
