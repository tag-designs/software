...
#ifndef SWD_DELAY_COUNT
#define SWD_DELAY_COUNT 1
#endif

#ifndef SWD_SPI_BR
#define SWD_SPI_BR (2U << 3)
#endif

#define DELCNT SWD_DELAY_COUNT
...
static bool spiPinsActive = false;
...
static void spiInit(void)
{
  ...
  SPI1->CR1 = 0;
  SPI1->CR2 = SPI_CR2_FRXTH | SPI_CR2_SSOE | (7U << 8);
  SPI1->CR1 = SPI_CR1_LSBFIRST | SWD_SPI_BR | SPI_CR1_MSTR;
...
static inline void _SetSWDIOasOutputBitBang(uint32_t size)
{
  (void)size;
  if (!isOutput || spiPinsActive)
  {
    palSetLine(LINE_SWDIO_DIR);
    delay(DELCNT);
    toOutput(LINE_TGT_SWCLK);
...
static inline void SW_ShiftOutBytes(uint32_t data, uint8_t bytes)
{
  ...
  while (rx < bytes)
  {
    if ((tx < bytes) && (SPI1->SR & SPI_SR_TXE))
      *(volatile uint8_t *)&SPI1->DR = buf[tx++];
...
static inline void SW_ShiftOut8(uint32_t data)
{
  SW_ShiftOutBytes(data, 1);
}

static inline void SW_ShiftOut16(uint32_t data)
{
  SW_ShiftOutBytes(data, 2);
}

static inline void SW_ShiftOut24(uint32_t data)
{
  SW_ShiftOutBytes(data, 3);
}

static inline void SW_ShiftOut32(uint32_t data)
{
