...
static bool rtc_enter_init(void) {
  ...
  for (uint32_t timeout = 10000U; timeout > 0U; timeout--) {
    if ((RTCD1.rtc->ICSR & RTC_ICSR_INITF) != 0U) {
      return true;
    }
  }
...
static bool rtc_monitor_connected(void) {

  return ((CoreDebug->DEMCR & CoreDebug_DEMCR_VC_CORERESET_Msk) != 0U) ||
...
void rtc_lld_init(void) {
  ...
  /* If calendar has not been initialized yet then proceed with the
     initial setup.*/
  if (!(RTCD1.rtc->ICSR & RTC_ICSR_INITS) && !rtc_monitor_connected()) {
