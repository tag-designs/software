...
It is not only Standby. The same part shows the same layout dependence in Stop 1 during normal running: with the MCU verifiably in Stop 1 for 97% of a 37 s window (LPMS=1, STOPF=1, 36.4 s of RTC_SSR dwell), the current drawn while asleep differs by ~200 uA between builds that differ only in code layout -- 671 vs 866 uA at 100 Hz, reproducible to 0.3 uA per build, with identical peripheral state latched at the instant of Stop entry and the whole 1 ms current distribution shifted rather than its duty cycle. Requesting Stop 2 instead removes the variation entirely: 605.0, 604.8, 604.9 uA across those same three layouts, and lower at every sample rate. So on this part the deeper the requested mode, the less reliably it is reached: Standby is a lottery that Stop 3 fixes, Stop 1 is a lottery that Stop 2 fixes.
