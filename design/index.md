...
*   [**CompassTag Power Test Plan**](../embedded/tags/families/CompassTag/design/power-test-plan.md): Hardware-in-the-loop verification of a Standby-after-attach idle-current regression on STM32L432 -- `DBGMCU->CR` written unconditionally instead of gated on `DHCSR.C_DEBUGEN`, leaving any board that had ever seen a debugger stuck around 365 µA instead of the ~376 nA a never-attached tag reaches.
*   [**CompassTag Power Test Report**](../embedded/tags/families/CompassTag/design/power-test-report.md): Results form and gate table for the CompassTag power test plan.
*   [**CompassTag Power Measurement Log**](../embedded/tags/families/CompassTag/design/power-test-results.md): Append-only record of every completed CompassTag power measurement.
*   [**UIUCTag Bring-Up Report**](../embedded/tags/families/BitPresTag/design/uiuctag-bringup-report.md): Six bugs found and fixed getting a fresh UIUCTag to real logged data -- an AT25XE write-cycle timing race, a same-thread SPI1 bus deadlock between the pressure sensor and external flash that explained both "no data logged" and "current never returns to idle", an ADXL367 loop-state reset gap, and more -- plus an extended power measurement.
*   [**IMUTag Start Abort Diagnostics**](../embedded/tags/families/IMUTag/design/start-abort-diagnostics.md): Open. Evidence for the intermittent abort at start and a proposal for a persistent per-marker detail word, using STM32U3 flash-row padding that STM32L4 targets do not have; touches the shared `t_StateMarker`, a weak hook in common `recordState()`, and one additive `State` proto field.
