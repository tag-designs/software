...
{   uint8_t cmdbuf[4];
    ...
    palClearLine(LINE_FLASH_nCS);
    spiExchange(&SPID1,4,cmdbuf,ret);
    //spiSendPolled(4, cmdbuf);
    spiReceive(&SPID1,num, buf);
    //spiReceivePolled(num, buf);
    palSetLine(LINE_FLASH_nCS);
}

static uint8_t devnull[32] NOINIT;
...
static void spi_cmd_addr_snd(uint8_t cmd, uint32_t address, uint8_t *buf, int num)
{
    ...
    spiExchange(&SPID1,4,cmdbuf,ret);
    //spiSendPolled(4, cmdbuf);
    if (num <= sizeof(devnull)) {
        spiExchange(&SPID1,num,buf,devnull);
    } else {
        spiSend(&SPID1,num, buf);
    }
    //
