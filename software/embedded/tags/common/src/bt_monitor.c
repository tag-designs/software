...
// Support for strings in Ack messages
...
static int data_logAck(int index)
{
  ...
  {
    if ((uint32_t)vddState[index].epoch == 0xffffffff)
      break;

    // Fill in message structure

    data[count].epoch = vddState[index].epoch;
    data[count].temperature = vddState[index].temp10 * 0.1f;
    data[count].voltage = vddState[index].vdd100 * 0.01f;
    data[count].rawdata = vddState[index].activity;
    count++;
    index++;
  }
...
int proto_eval(int len)
{
  ...
  case Req_log_tag:
    ...
    case LogReq_INTERNAL_DATA:
      return data_logAck(req.payload.log.index);
