...
bool ExFlashWrite(uint32_t address, uint8_t *buf, int *cnt)
{
    ...
    spi_cmd_addr_snd(AT25XE_CMD_SEQ_PROGRAM,address,buf,1);
    chThdSleepMicroseconds(400);
    buf++;
    num--;
    while (num--) {
        spi_cmd_snd(AT25XE_CMD_SEQ_PROGRAM,buf,1);
        for (int i = 0; i < 10; i++)
        {
    ...
    *cnt = 0;
    spi_cmd(AT25XE_CMD_WRITE_DISABLE);
    return true;
}
/*
bool ExFlashWrite(uint32_t address, uint8_t *buf, int *cnt)
